...
var fapmeter;
var fapmeter_nosock = "<?php echo $c->getFileUrlFromTheme('smileys/nosock.png'); ?>";
var fapmeter_sock = "<?php echo $c->getFileUrlFromTheme('smileys/sock.png'); ?>";
var fapmeter_halfsock = "<?php echo $c->getFileUrlFromTheme('smileys/halfsock.png'); ?>";
var fapmeter_padding = 3;

function showFapmeter(event)
{
	if(fapmeter == undefined)
	{
		fapmeter = document.createElement('div');

		fapmeter.onclick = function() {
			pfc.sendRequest('/send '+ getFapString());
		...
		fapmeter.style.zIndex = '400';
		fapmeter.style.position = 'absolute';
		fapmeter.style.padding = fapmeter_padding + 'px';

		for(i=0;i<5;i++)
		{
			img = document.createElement('img');
			img.setAttribute('src', fapmeter_nosock);
			fapmeter.appendChild(img);
		}
	}

	if(fapmeter.style.display == 'block')
	{
		fapmeter.style.display = 'none';
		return;
	}

	fapmeter.style.display = 'block';
	fapmeter.style.left = (mousePosX(event)+7) + 'px';
	fapmeter.style.top = (mousePosY(event)-14) + 'px'; 

	document.body.appendChild(fapmeter);
}

function setSocks(value)
{
	value -= fapmeter_padding;
	socks = value / 18 + 0.2;

	for(i=1;i<=5;i++)
	{
		curSock = fapmeter.children[i - 1];
		if(i<socks)
			curSock.src = fapmeter_sock;
		else
		{
			if(i - socks > 0.5)
				curSock.src = fapmeter_nosock;
			else
				curSock.src = fapmeter_halfsock;
		}
	}
}

function getFapString()
{
	result = "";

	faps = Math.floor(socks);
	half = socks - faps > 0.5;
	nofaps = Math.round(5 - socks);
